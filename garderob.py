...
def generate_advice(name, category):
    ...
    if "джинс" in text:
        if "клеш" in text:
            return "Подойдут свободная футболка, свитер, кроссовки и спокойный верхний слой."
        return "Подойдут базовая футболка, оверсайз-свитер и кроссовки."
    if "кроссовк" in text or "кед" in text:
        return "Хорошо смотрятся с джинсами, шортами, худи или простым спортивным комплектом."
    if "худи" in text:
        return "Лучше всего сочетается с джинсами, широкими брюками и кроссовками."
    if "пиджак" in text:
        return "Можно носить с брюками, джинсами, рубашкой или базовой футболкой."
    if "сланц" in text or "тапк" in text:
        return "Подойдут шорты, майка или футболка — вариант для дома, отдыха или пляжа."
    tips = {
